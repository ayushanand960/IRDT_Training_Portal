...
const AllUsersPage = () => {
  ...
  const branchOptions = [...new Set(users.map((u) => u.branch).filter(Boolean))];
  const instituteOptions = [...new Set(users.map((u) => u.institute_name).filter(Boolean))];
  const designationOptions = [...new Set(users.map((u) => u.designation).filter(Boolean))];

  // Apply filters
  const filteredUsers = users.filter((u) => {
    return (
      (branchFilter ? u.branch === branchFilter : true) &&
      (instituteFilter ? u.institute_name === instituteFilter : true) &&
      (designationFilter ? u.designation === designationFilter : true)
    );
  });
            ...
            <tbody>
              {filteredUsers.map((user, index) => (
                <tr key={user.ehrms_code}>
                  ...
                  <td>
                    <Link
                      to={`/users/${user.ehrms_code}/trainings`}
