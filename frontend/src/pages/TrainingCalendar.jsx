...
const TrainingCalendar = () => {
  ...
  const [visibleCounts, setVisibleCounts] = useState({}); // for "Show More" tracking
  ...
  useEffect(() => {
    ...
    const grouped = [
      { section: '🟢 Trainings This Week', items: thisWeek.sort((a, b) => new Date(a.start_date) - new Date(b.start_date)) },
      { section: '🟡 Upcoming Week Trainings', items: upcoming.sort((a, b) => new Date(a.start_date) - new Date(b.start_date)) },
      { section: '🔴 Past Trainings', items: past.sort((a, b) => new Date(b.start_date) - new Date(a.start_date)) },
    ];

    setFilteredTrainings(grouped);

    // Reset visible counts (12 per group initially)
    const defaultCounts = {};
    grouped.forEach(g => {
      defaultCounts[g.section] = 6;
    });
  ...
  return (
    <div className="container py-4">
      ...
      {Array.isArray(filteredTrainings) && filteredTrainings.every(group => group.items.length === 0) ? (
        <p className="text-center text-muted mt-4">No trainings found.</p>
      ) : (
        filteredTrainings.map((group, idx) => {
          if (!group.items || group.items.length === 0) return null;

          const visible = visibleCounts[group.section] || 12;
          ...
          return (
            <div key={idx} className="mb-5">
              <h5 className="border-bottom pb-2">{group.section}</h5>
              <Row className="g-3">
                {group.items.slice(0, visible).map((training, index) => (
              ...
              {hasMore && (
                <div className="text-center mt-3">
                  <Button variant="outline-primary" onClick={() => handleShowMore(group.section)}>
                    Show More
                  </Button>
                </div>
              )}
